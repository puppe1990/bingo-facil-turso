...
export function Layout({ children }: { children?: ReactNode }) {
  ...
  const sidebarAccessHint =
    userAccess?.effectiveStatus === 'inactive'
      ? 'Aguardando ativação'
      : userAccess?.effectiveStatus === 'expired'
        ? 'Acesso expirado'
        : userAccess?.effectiveStatus === 'active'
          ? 'Acesso ativo'
          : 'Carregando...';
        ...
        <div className="p-6 bg-indigo-900 mt-auto">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-indigo-300 truncate mt-1">{sidebarAccessHint}</p>
            </div>
          </div>
          <button
