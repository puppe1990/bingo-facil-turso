...
export function LiveDraw() {
  ...
  if (loading || !event) return <div className="flex justify-center py-20"><RotateCcw className="animate-spin" /></div>;
      ...
      <div className="fixed inset-0 bg-indigo-900 text-white p-12 flex flex-col font-sans overflow-hidden z-[100]">
        <div className="flex justify-between items-center mb-12">
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 bg-amber-400 rounded-full flex items-center justify-center text-indigo-900 font-black text-3xl shadow-xl">B</div>
            <div>
          ...
          <div className="flex gap-4">
            ...
            <button onClick={() => setIsProjection(false)} className="bg-white/10 px-6 py-3 rounded-xl hover:bg-white/20 font-bold transition-all text-sm uppercase tracking-widest">
              Sair do Telão
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-12 items-center">
          <div className="col-span-5 flex flex-col items-center">
            <div className="text-indigo-300 text-2xl font-black mb-8 uppercase tracking-[0.3em]">Última Pedra</div>
            <motion.div
              key={lastNumber}
            ...
            >
              ...
              {lastNumber && <span className="text-7xl font-black opacity-60 leading-none mb-2">{letters[Math.floor((lastNumber-1)/15)]}</span>}
              <span className="text-[180px] leading-none font-black tracking-tighter">{lastNumber || '--'}</span>
            </motion.div>
            
            <div className="mt-16 w-full max-w-md">
              <div className="text-indigo-300 text-sm font-black mb-6 uppercase tracking-widest text-center">🏆 Ganhadores em Tempo Real</div>
              <div className="space-y-4">
                <AnimatePresence>
                  {winners.map(w => (
                    ...
                    >
                      ...
                      <div>
                        <div className="text-xs font-black opacity-80 uppercase tracking-widest italic">BINGO! Cartela #{w.cardNumber}</div>
                ...
                {winners.length === 0 && (
                  <div className="text-center text-indigo-400 font-bold italic opacity-40">Aguardando primeiro vencedor...</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="col-span-7 bg-indigo-950/60 p-10 rounded-[3rem] border-2 border-white/5 shadow-2xl backdrop-blur-md">
            <div className="grid grid-cols-5 gap-4 mb-8">
               {letters.map((l, idx) => (
                 <div key={l} className="text-4xl font-black text-center text-indigo-400/50">{l}</div>
               ))}
            </div>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <Link to="/event/$eventId" params={{ eventId }} className="inline-flex items-center gap-2 text-indigo-400 hover:text-indigo-600 mb-2 font-bold text-sm bg-indigo-50 px-3 py-1 rounded-full transition-all">
            <ArrowLeft className="w-4 h-4" /> Painel do Evento
          </Link>
          <h1 className="text-4xl font-black text-indigo-900 uppercase tracking-tight">Sorteio ao Vivo</h1>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
           <button 
             ...
             className={`p-4 rounded-2xl border-2 transition-all shadow-sm ${
               soundEnabled ? 'bg-white border-indigo-100 text-indigo-600' : 'bg-gray-100 border-gray-200 text-gray-400'
             }`}
           >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-8 space-y-8">
          <div className="bg-white p-12 rounded-[3.5rem] border-2 border-indigo-50 shadow-2xl shadow-indigo-100/50 flex flex-col items-center relative overflow-hidden">
             ...
             <div className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.4em] mb-8">Status: Rodada em Andamento</div>
             ...
             <div className="relative flex items-center justify-center group">
               ...
               >
                  <span className="text-4xl font-black opacity-60 leading-none mb-1">{lastNumber ? letters[Math.floor((lastNumber-1)/15)] : ''}</span>
                  <span className="text-[120px] leading-none font-black tracking-tighter">{lastNumber || '--'}</span>
               </motion.div>
             </div>
             ...
             >
                <div className="bg-white/20 p-2 rounded-lg"><Play className="fill-current w-6 h-6" /></div>
                Sortear Pedra
             </button>
             
             <div className="mt-10 flex gap-12 text-sm font-black">
                <div className="flex flex-col items-center">
                  <span className="text-indigo-300 uppercase tracking-widest text-[10px] mb-1">Sorteados</span>
                  <span className="text-2xl text-indigo-900">{drawnNumbers.length}</span>
                </div>
                <div className="flex flex-col items-center">
                  <span className="text-indigo-300 uppercase tracking-widest text-[10px] mb-1">Restantes</span>
             ...
             <div className="mt-12 pt-8 border-t border-indigo-50 w-full max-w-md">
                <div className="text-xs font-black text-indigo-300 uppercase tracking-widest mb-4 text-center">Conferência Rápida</div>
                <div className="relative group">
                   <div className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-300 group-focus-within:text-indigo-600 transition-colors">#</div>
                   <input 
                     type="text" 
                     ...
                     onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          ...
                          if (found) {
                            ...
                            alert(isWin ? `🏆 BINGO! A cartela ${num} (${found.buyerName}) ESTÁ PREMIADA!` : `❌ Ainda não... A cartela ${num} possui números extras pendentes.`);
                          } else {
        ...
        <div className="xl:col-span-4 space-y-6">
          <div className="bg-rose-500 p-8 rounded-[2.5rem] text-white shadow-2xl shadow-rose-100 relative overflow-hidden min-h-[400px]">
            ...
            {winners.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 text-center space-y-4 relative">
                ...
                <p className="text-sm font-bold text-rose-100 uppercase tracking-widest leading-relaxed">Aguardando o<br/>Grito de Bingo!</p>
              </div>
            ) : (
              <div className="space-y-4 relative">
                {winners.map(w => (
                  ...
                  >
                    <div className="text-[10px] font-black text-amber-300 uppercase tracking-[0.2em] mb-1">Bingo Confirmado!</div>
          ...
          <div className="bg-indigo-900 p-8 rounded-[2.5rem] text-white shadow-xl relative overflow-hidden">
             <div className="grid grid-cols-2 gap-6">
                <div>
                  <div className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Tipo Bingo</div>
                  <div className="text-lg font-black text-amber-400">75 Pedras</div>
                </div>
                <div>
                  <div className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Voz Ativa</div>
                  <div className="text-lg font-black text-emerald-400">{soundEnabled ? 'Sim' : 'Não'}</div>
                </div>
             </div>
             <div className="mt-8 pt-6 border-t border-indigo-800 flex justify-between items-center">
                <div className="text-xs font-bold text-indigo-300">Dica: Aperte ESPAÇO para sortear</div>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-10 rounded-[3rem] border-2 border-indigo-50 shadow-sm">
         <div className="flex items-center justify-between mb-8">
            <h3 className="text-sm font-black text-indigo-300 uppercase tracking-[0.4em]">Painel Completo de Pedras</h3>
            <div className="flex gap-2">
               {letters.map(l => (
                 <div key={l} className="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center text-xs font-black text-indigo-400">{l}</div>
               ))}
            </div>
         </div>
         <div className="grid grid-cols-10 sm:grid-cols-15 gap-3">
            {Array.from({ length: 75 }).map((_, i) => {
              ...
              return (
                <div key={n} className={`aspect-square flex items-center justify-center rounded-xl text-lg font-black transition-all duration-300 ${
                  active 
                    ? isLast 
