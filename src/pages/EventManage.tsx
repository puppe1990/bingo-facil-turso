...
export function EventManage() {
  ...
  const handleSellCard = async (cardId: string, buyerName: string, buyerPhone: string) => {
    ...
    try {
      ...
      setCards((prev) =>
        prev.map((c) => (c.id === cardId ? { ...c, status: 'sold', buyerName, buyerPhone } : c)),
      );
  ...
  const filteredCards = cards.filter(
    (c) =>
      c.cardNumber.includes(search) || c.buyerName?.toLowerCase().includes(search.toLowerCase()),
  );
  ...
  if (loading || !event)
    return (
      <div className="flex justify-center py-12 text-indigo-600 animate-spin">
    ...
    <div className="space-y-10 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        <div>
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-indigo-400 hover:text-indigo-600 mb-4 font-bold text-sm bg-indigo-50 px-3 py-1 rounded-full transition-all"
          >
            <ArrowLeft className="w-4 h-4" /> Dashboard
          </Link>
          <h1 className="text-5xl font-black text-indigo-900 uppercase tracking-tighter leading-none">
            {event.name}
          </h1>
          ...
          <div className="relative">
            ...
            {showPdfOptions && (
              <div className="absolute top-full mt-2 right-0 w-64 bg-white rounded-2xl shadow-2xl border-2 border-indigo-50 p-4 z-20 animate-in fade-in slide-in-from-top-2">
                <p className="text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-4">
                  Cartelas por Página
                </p>
        ...
        <div className="bg-indigo-900 p-8 rounded-[2.5rem] text-white shadow-2xl shadow-indigo-200 border-2 border-indigo-800 flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-4xl font-black text-white">
              {Math.round((soldCount / event.totalCards) * 100)}%
            </p>
          </div>
      ...
      <div className="bg-white rounded-[3rem] border-2 border-indigo-50 shadow-2xl shadow-indigo-100/50 overflow-hidden relative">
        <div className="p-8 border-b-2 border-indigo-50 bg-indigo-50/20 flex flex-col sm:flex-row gap-6 items-center justify-between">
          <h2 className="text-2xl font-black text-indigo-900 uppercase italic">
            Controle de Vendas
          </h2>
            ...
            <tbody className="divide-y-2 divide-indigo-50">
              {filteredCards.map((card) => (
                <tr key={card.id} className="hover:bg-indigo-50/30 transition-colors group">
                  <td className="px-8 py-6 font-black text-indigo-900 text-lg">
                    #{card.cardNumber}
                  </td>
                  <td className="px-8 py-6">
                    <span
                      className={`text-[10px] uppercase font-black px-3 py-1.5 rounded-lg tracking-widest ${
                        card.status === 'sold'
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      ...
                      {card.buyerPhone && (
                        <span className="text-xs text-indigo-400 font-bold">{card.buyerPhone}</span>
                      )}
                    </div>
                  </td>
                          ...
                          <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter')
                                    handleSellCard(card.id, buyerNameInput, buyerPhoneInput);
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-indigo-950/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] shadow-2xl p-8 max-w-md w-full border-2 border-indigo-50 animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-2xl font-black text-indigo-900 text-center mb-2 uppercase italic">
              Atenção!
            </h3>
            <p className="text-indigo-400 text-center font-medium leading-relaxed mb-8">
              Você tem certeza que deseja excluir o evento{' '}
              <span className="font-bold text-indigo-900">"{event.name}"</span>? Esta ação é
              permanente e todas as cartelas e dados do sorteio serão perdidos.
            </p>
